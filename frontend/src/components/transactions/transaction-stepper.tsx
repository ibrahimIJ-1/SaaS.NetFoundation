...
import { z } from 'zod';
import { 
  CheckCircle2, Circle, Clock, ChevronDown, ChevronUp, FileText, 
  DollarSign, Loader2, AlertCircle, Upload, RotateCcw, Trash2 
} from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import { Textarea } from '@/components/ui/textarea';
import { LegalTransaction, TransactionStep, StepStatus } from '@/types/workflow';
import { useUpdateStep, useCancelTransaction, useUploadTransactionFile, useDeleteTransactionFile } from '@/hooks/use-workflows';
import { cn } from '@/lib/utils';
import { Badge } from '@/components/ui/badge';

const stepUpdateSchema = z.object({
  actualPrice: z.coerce.number().min(0),
  actualExpense: z.coerce.number().min(0),
...
const statusConfig: Record<StepStatus, { label: string; color: string; icon: React.ElementType }> = {
  Pending: { label: 'في الانتظار', color: 'text-muted-foreground', icon: Circle },
...
function StepPanel({ step, transactionId, isActive }: StepPanelProps) {
  ...
  const { mutate: uploadFile, isPending: uploading } = useUploadTransactionFile(transactionId);
  ...
  const handleSave = (data: StepUpdateForm) => {
    updateStep({ stepId: step.id, data }, {
      onSuccess: () => setIsEditing(false)
    });
  };

  const handleComplete = (data: StepUpdateForm) => {
    updateStep({ stepId: step.id, data: { ...data, status: 'Completed' } }, {
      onSuccess: () => setIsEditing(false)
    });
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3 text-xs text-muted-foreground">
          <span className="hidden sm:block">
            سعر: <span className="font-medium text-foreground">{step.actualPrice.toLocaleString('ar-IQ')}</span>
          </span>
          <span className="hidden sm:block">
            مصاريف: <span className="font-medium text-amber-400">{step.actualExpense.toLocaleString('ar-IQ')}</span>
          </span>
      ...
      {/* Step Body */}
      {expanded && (
        <div className="px-4 pb-4 space-y-4 border-t border-border/50 pt-4" dir="rtl">
          ...
          {!isEditing && step.status === 'Completed' && (
            <div className="flex items-center justify-between p-2 bg-emerald-500/10 rounded-lg">
              <span className="text-xs text-emerald-400 font-medium">تم إكمال هذه الخطوة</span>
              <Button 
                variant="ghost" 
                size="sm" 
          ...
          <form onSubmit={form.handleSubmit(handleComplete)} className="space-y-4">
            ...
            {step.requiredFileNames?.length > 0 && (
              <div className="space-y-1">
                <p className="text-xs font-medium text-muted-foreground">المستندات المطلوبة:</p>
                <div className="flex flex-wrap gap-2">
                  {step.requiredFileNames.map((f, i) => (
                    <span key={i} className="flex items-center gap-1 bg-secondary text-secondary-foreground text-xs px-2 py-1 rounded-md">
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                {step.uploadedFilesJson ? (
                  JSON.parse(step.uploadedFilesJson).map((file: string, i: number) => (
                    <div key={i} className="flex items-center gap-2 p-2 bg-secondary/50 rounded-md border border-border group relative">
                      <FileText className="w-4 h-4 text-legal-gold" />
                      <span className="text-xs truncate max-w-[150px]">{file}</span>
                      {isEditing && (
                        <Button
                          type="button"
                          variant="ghost"
                          ...
                          onClick={() => deleteFile({ stepId: step.id, fileName: file })}
                        >
                  ...
                  <p className="text-xs text-muted-foreground italic">لا توجد ملفات مرفوعة.</p>
                )}
            ...
            <div className="flex gap-4 p-3 bg-secondary/30 rounded-lg text-xs">
              <div>
                ...
                <span className="font-medium">{step.estimatedPrice.toLocaleString('ar-IQ')}</span>
              </div>
              <div>
                <span className="text-muted-foreground">المصاريف التقديرية: </span>
                <span className="font-medium">{step.estimatedExpense.toLocaleString('ar-IQ')}</span>
              </div>
              <div className="mr-auto">
                <span className="text-muted-foreground">صافي الربح الفعلي: </span>
                <span className={cn('font-bold', profit >= 0 ? 'text-emerald-400' : 'text-red-400')}>
                  {profit.toLocaleString('ar-IQ')}
                </span>
              </div>
            </div>
            ...
            {isEditing && (
              <div className="flex gap-2 justify-end">
                ...
                <Button
                  ...
                  disabled={isPending}
                >
                  {isPending ? <Loader2 className="w-3 h-3 animate-spin ml-1" /> : <CheckCircle2 className="w-3 h-3 ml-1" />}
                  {step.status === 'Completed' ? 'تحديث وإنهاء' : 'إنهاء الخطوة'}
                </Button>
              </div>
            )}

            {!isEditing && step.status === 'Completed' && step.completionDate && (
              <p className="text-xs text-emerald-400 text-left">
                ✓ اكتملت في {new Date(step.completionDate).toLocaleDateString('ar-IQ')}
              </p>
            )}
          </form>
        </div>
...
export function TransactionStepper({ transaction }: TransactionStepperProps) {
  ...
  const totalActualPrice = transaction.steps.reduce((s, step) => s + step.actualPrice, 0);
  const totalExpenses = transaction.steps.reduce((s, step) => s + step.actualExpense, 0);
  const netProfit = totalActualPrice - totalExpenses;
  const progress = transaction.steps.length > 0
    ? Math.round((transaction.steps.filter((s) => s.status === 'Completed').length / transaction.steps.length) * 100)
    : 0;
    ...
    <div className="space-y-6" dir="rtl">
      ...
      <div className="bg-card border border-border rounded-xl p-5 space-y-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-xl font-bold text-foreground">{transaction.transactionNumber}</h2>
              <Badge className={cn('border text-xs', txStatusColors[transaction.status])}>
                {transaction.status === 'Active' ? 'نشطة' : transaction.status === 'Completed' ? 'مكتملة' : 'ملغاة'}
              </Badge>
            </div>
            <p className="text-sm text-muted-foreground mt-0.5">{transaction.workflowName} — {transaction.contactName}</p>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { label: 'السعر المتفق', value: transaction.actualPrice, color: 'text-foreground' },
            { label: 'مجموع المصاريف', value: totalExpenses, color: 'text-amber-400' },
            { label: 'السعر التقديري', value: transaction.estimatedPrice, color: 'text-muted-foreground' },
            { label: 'صافي الربح', value: netProfit, color: netProfit >= 0 ? 'text-emerald-400' : 'text-red-400' },
          ].map((stat) => (
